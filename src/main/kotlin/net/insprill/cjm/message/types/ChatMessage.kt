...
class ChatMessage(private val plugin: CustomJoinMessages) : MessageType(plugin, "chat", "Messages") {
    ...
    override fun handle(primaryPlayer: Player, recipients: List<Player>, chosenPath: String, visibility: MessageVisibility) {
        ...
        if (visibility == MessageVisibility.PUBLIC && config.getBoolean(MessageVisibility.PUBLIC.configSection + ".Send-To-Console")) {
            for (message in messages) {
                plugin.server.consoleSender.sendMessage(TextComponent.toLegacyText(*message))
            }
        }
    }
    ...
    /**
     * Taken and modified from [this](https://www.spigotmc.org/threads/95872/) thread on SpigotMC.org.
     *
     * @author SirSpoodles
     */
    private object CenteredMessages {
        ...
        fun centerMessage(message: String): String {
            ...
            for (element in message) {
                if (element == '§' || element == '&') {
                    ...
                    previousCode = false
                    isBold = element == 'l' || element == 'L'
                } else {
                    val dFI: DefaultFontInfo = DefaultFontInfo.getDefaultFontInfo(element)
        ...
        enum class DefaultFontInfo(val character: Char, val length: Int) {
            ...
            val boldLength get() = if (this == SPACE) length else length + 1
